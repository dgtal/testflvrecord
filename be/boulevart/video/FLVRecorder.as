...
//-  METHODS -----------------------------------------------------------------------------
	 ...
	 * @param fps Framerate
	 * @param stage Instance of your stage, needed to do some fake threading
	 * @param durationInSeconds Duration of video, in seconds, if not filled in, will be automaticly determined
	 * 
	 */	
	public function setTarget(file:File, width:int, height:int, fps:Number, stage:Stage,durationInSeconds:Number=0):void
		{
			/*
				Parameters:
		...
		private function doStop():void{
			...
			//		ba.uncompress()
					...
					var dummy:Sprite=new Sprite()
					stg.addChild(dummy)
					new PseudoThread(dummy,writeFrame,{bmp:frame})	
				/*}else{
